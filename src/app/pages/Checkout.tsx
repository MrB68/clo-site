  ...
  try {
    ...
    for (const item of cartItems) {
      ...
      if (available < item.quantity) {
        toast.error(`Only ${available} left for size ${item.size}`);
        setIsProcessingPayment(false);
        return;
      }
    }
    ...
    const fields = {
      ...
      signed_field_names: "total,transaction_uuid,product_code",
  ...
  const finalizeOrder = async () => {
    ...
    try {
      ...
      for (const item of storedOrder.items || []) {
        ...
        const sizeStock = product?.size_stock || {};
        const currentSizeStock = Number(sizeStock[item.size] ?? 0);
        ...
        const totalStock = Object.values(updatedSizeStock).reduce(
      ...
      // 🔥 UPDATE SALES + ORDER COUNTS
      for (const item of storedOrder.items || []) {
        try {
          const { data: product } = await supabase
            .from("products")
            .select("sales_count, orders_count")
            .eq("id", item.id)
            .single();

          const currentSales = Number(product?.sales_count || 0);
          const currentOrders = Number(product?.orders_count || 0);

          await supabase
            .from("products")
            .update({
              sales_count: currentSales + item.quantity,
