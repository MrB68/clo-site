...
export function ProductManagement() {
  ...
  // Force dark mode globally when this admin page loads
  useEffect(() => {
...
setProducts(normalized);
  ...
  const saveProducts = async (updatedProducts: Product[]) => {
    try {
      const dbProducts = updatedProducts.map((product) => ({
        id: product.id,
        name: product.name,
        price: product.price,
  ...
  const handleAddProduct = async () => {
    ...
    const newProduct: any = {
      ...
      stock: Object.values(formData.sizeStock || {}).reduce(
        (sum, v) => sum + Number(v || 0),
        0
      ),
      size_stock: Object.fromEntries(
        Object.entries(formData.sizeStock || {}).map(([k, v]) => [k, Number(v)])
      ),
    ...
    const uiProduct: Product = {
      ...
      sizeStock: newProduct.size_stock || {},
    };
  ...
  const handleEditProduct = async () => {
    ...
    // --- Ensure size_stock is always present and never null ---
    const safeSizeStock = Object.fromEntries(
      Object.entries(formData.sizeStock || {}).map(([k, v]) => [k, Number(v || 0)])
    );
    // Optional strict validation: prevent empty size stock from updating
    if (!Object.keys(safeSizeStock).length) {
      toast.error("Size stock is required");
      return;
    }
  ...
  const startEdit = (product: Product) => {
    ...
    setFormData({
      ...
      sizeStock: Object.fromEntries(
        Object.entries(product.sizeStock || {}).map(([k, v]) => [k, String(v)])
      ),
      customColorInput: "",
    });
  };
...
saveProducts(reordered);
      ...
      >
<SortableContext items={sortedProducts.map(p => p.id)} strategy={rectSortingStrategy}>          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    <Card className="h-full flex flex-col dark:bg-zinc-800 dark:border-zinc-700 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                      <CardHeader className="pb-3 relative">
                        <div
                          {...attributes}
                          {...listeners}
                          className="absolute top-3 left-3 cursor-grab active:cursor-grabbing p-1 text-gray-400 hover:text-white transition z-10"
                        >
                        ...
                        <div className="flex items-start justify-between gap-2 w-full">
                          <div className="flex-1 min-w-0 overflow-hidden pl-6">
                            ...
                            {/* Action icons below product name, right aligned */}
                            <div className="flex justify-end mt-2 gap-1">
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => toggleFeature(product.id)}
                                className={`flex-shrink-0 ${product.featured ? "text-yellow-600" : ""}`}
                              >
                                {product.featured ? <Eye size={16} /> : <EyeOff size={16} />}
                              </Button>
                              <Button
                                variant="ghost"
                                size="icon"
                              ...
                              <AlertDialog>
                                <AlertDialogTrigger asChild>
                                  <Button variant="ghost" size="icon" className="text-red-600 hover:text-red-700 flex-shrink-0">
                                    <Trash2 size={16} />
                                  </Button>
                                </AlertDialogTrigger>
                                <AlertDialogContent>
...
function ProductForm({
  ...
  // --- Custom toggleSize for cleaning sizeStock ---
  const safeToggleSize = (size: string) => {
    setFormData(prev => {
      ...
      if (isRemoving) delete newSizeStock[size];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          {!formData.category && (
            <p className="text-xs text-red-500">Select a category</p>
          )}
        </div>
        <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          {(!formData.price || Number(formData.price) <= 0) && (
            <p className="text-xs text-red-500">Enter a valid price</p>
          )}
        </div>
        <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          {/* Inline error for Sizes */}
          {formData.sizes.length === 0 && (
            <p className="text-xs text-red-500">Select at least one size</p>
          )}
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-zinc-700">
        ...
        <Button
          onClick={onSubmit}
          disabled={!isValid}
          className={`transition-all ${!isValid ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          {submitLabel}
        </Button>
      </div>
