...
useEffect(() => {
  ...
  // Filter by style (only if style exists)
  filteredProducts = filteredProducts.filter((p) => {
    if (!p.style) return true; // allow products without style
    return p.style === selectedStyle;
  });
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="animate-pulse space-y-4">
                    <div className="h-[420px] bg-neutral-800" />
              ...
              >
                {filteredProducts.map((product) => (
                  <motion.div
                    ...
                    className="group bg-neutral-950 p-5 transition-all duration-500 hover:shadow-2xl border border-white/10"
                  >
