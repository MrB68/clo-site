...
export function Cart() {
  ...
  const [sizeStockMap, setSizeStockMap] = useState<Record<string, Record<string, number>>>({});
  ...
  useEffect(() => {
    ...
    const syncCart = () => {
      ...
      setCartItems((prev) => {
        ...
        // avoid unnecessary re-render (fixes ghost glitch)
  ...
  useEffect(() => {
    const fetchStock = async () => {
      ...
      data?.forEach((p: any) => {
        ...
        sizeMap[p.id] = p.size_stock || {};
      });

      setStockMap(map);
      setSizeStockMap(sizeMap);
    };
  ...
  const updateQuantity = (index: number, delta: number) => {
    setCartItems((items) =>
      items.map((item, i) => {
        ...
        const available = sizeStockMap[item.productId]?.[item.selectedSize] ?? 0;
  ...
  const hasInvalidQty = populatedCartItems.some(
    (item) =>
      item.quantity >
      (sizeStockMap[item.productId]?.[item.selectedSize] ?? 0)
  );
        ...
        <AnimatePresence>
          {populatedCartItems.map((item, index) => (
            <motion.div
              ...
              exit="exit"
              layout
              className="flex gap-6 border-b border-black/10 pb-6 dark:border-white/10 hover:scale-[1.01] transition-transform"
            >
              {/* Image */}
              <Link
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {(sizeStockMap[item.productId]?.[item.selectedSize] ?? 0) === 0
                      ? "Out of stock"
                      : `Available: ${sizeStockMap[item.productId]?.[item.selectedSize] ?? 0}`}
                  </p>
                </div>
                ...
                <div className="flex justify-between items-center mt-4">
                  ...
                  <div className="flex border border-black/15 dark:border-white/15">
                    ...
                    <button
                      onClick={() => updateQuantity(index, 1)}
                      disabled={(sizeStockMap[item.productId]?.[item.selectedSize] ?? 0) <= item.quantity}
                      className={`px-3 py-2 transition-colors ${
                        (sizeStockMap[item.productId]?.[item.selectedSize] ?? 0) <= item.quantity
                  ...
                  {item.quantity > (sizeStockMap[item.productId]?.[item.selectedSize] ?? 0) && (
